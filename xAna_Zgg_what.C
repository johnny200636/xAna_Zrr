...
void xAna_Zgg_what(Int_t MC_N, Int_t MC, Int_t WP=1) {
  
  // if MC == 0, it's S10
  // if MC == 1, it's RD1

  rochcor2012 *rmcor = new rochcor2012(229);
  zgamma::PhosphorCorrectionFunctor* obj = new zgamma::PhosphorCorrectionFunctor("/afs/cern.ch/user/c/ctu/CMSSW_5_3_7/src/MyPhosphorDir/PHOSPHOR_NUMBERS_EXPFIT_ERRORS.txt", true);
  //Double_t   forprin[6]   = {15,20,30,40,50,120};

  TH1* Zmass             = new TH1D("Zmass","Z mass", 35, 50, 120);
  TH1* Zgamma_mass       = new TH1D("Zgamma_mass","Zgamma mass", 125, 50, 300);
  TH1* Zgammagamma_mass  = new TH1D("Zgammagamma_mass","Zgammagamma mass", 125, 50, 300);
  TH1* two_mass          = new TH1D("two_mass","two photons mass", 150, 0, 300);
  TH1* Photon1_Pt         = new TH1D("Photon1_Pt","Photon1_Pt", 30, 0, 300);
  TH1* Photon2_Pt         = new TH1D("Photon2_Pt","Photon2_Pt", 30, 0, 300);
  TH1* Number_event      = new TH1D("Number of event","Number of event by selecting different photons", 5, 0, 5);

  Char_t fname_File[30];
  //sprintf(fname_File,"Result/Zrr_%d_EnCorr.root",MC_N);
  //sprintf(fname_File,"Result/Zrr_%d_Sa_new.root",MC_N);
  sprintf(fname_File,"Result/test.root");
  ...
  //RD1  
  //if(MC_N==1)    pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_DYJetsToLL.root");  
  //if(MC_N==2)    pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_Zg.root");
  //if(MC_N==3)    pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_ZZ_2e2mu.root");

  //MAGRAPH
  if(MC_N==1){pathes.push_back("/data5/ggNtuples/V05-03-12-03/job_summer12_DYJetsToLL_s10.root");}
  //
  if(MC_N==21){pathes.push_back("/data5/ggNtuples/V05-03-07-05/job_summer12_Zg.root");}
  if(MC_N==22){pathes.push_back("/data2/ctu/Zgg/Zg/ZToLL/ZToLL.root");}
  if(MC_N==221){pathes.push_back("/data2/ctu/Zgg/Zg/ZToLL/ZToLL.root");}
  //
  if(MC_N==3){pathes.push_back("/data5/ggNtuples/V05-03-07-11/job_summer12_ZZ_2e2mu.root");}
  if(MC_N==4){pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_ZZ_2l2nu.root");}
  if(MC_N==5){pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_ZZ_2l2q.root");}
  if(MC_N==6){pathes.push_back("/data5/ggNtuples/V05-03-11-00/job_summer12_ZZ_2q2nu.root");}
  if(MC_N==7){pathes.push_back("/data5/ggNtuples/V05-03-12-05/job_summer12_Zgg.root");}
  
  if(MC_N==0){
	pathes.push_back("/data3/ggNtuples/V05-03-12-03/job_2muon_2012a_Jan22rereco.root");
	pathes.push_back("/data3/ggNtuples/V05-03-12-03/job_2muon_2012b_Jan22rereco.root");
	pathes.push_back("/data3/ggNtuples/V05-03-12-03/job_2muon_2012c_Jan22rereco.root");
	pathes.push_back("/data3/ggNtuples/V05-03-12-03/job_2muon_2012d_Jan22rereco.root");
  }	
  
  if(MC_N==8){pathes.push_back("root://eoscms//eos/cms/store/group/phys_egamma/cmkuo/job_summer12_ttjets_1l.root");}
  if(MC_N==9){pathes.push_back("root://eoscms//eos/cms/store/group/phys_egamma/cmkuo/job_summer12_ttjets_2l.root");}
  if(MC_N==10){pathes.push_back("root://eoscms//eos/cms/store/group/phys_egamma/cmkuo/job_summer12_Wjets.root");}
  //POWHEG
  if(MC_N==11){pathes.push_back("/data5/ggNtuples/V05-03-12-06/job_summer12_DYToMuMu_s10.root");}

  if(MC_N==12){pathes.push_back("/data5/ggNtuples/V05-03-07-11/job_summer12_WZ_2l2q.root");}
  if(MC_N==13){pathes.push_back("/data5/ggNtuples/V05-03-07-11/job_summer12_WZ_3lnu.root");}
  if(MC_N==14){pathes.push_back("/data5/ggNtuples/V05-03-07-05/job_summer12_WW_2l2nu.root");}
  if(MC_N==15){pathes.push_back("/data2/ctu/Zgg/job_summer12_ZToTauTau.root");}
  ...
  */
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++) {
    ...
    */
    ...
    Float_t* phoSCRChIso = data.GetPtrFloat("phoSCRChIso");
    Float_t  rho2012     = data.GetFloat("rho2012");
    Float_t* phoR9     = data.GetPtrFloat("phoR9");
      
    Float_t   phoRandConeChIso = 0;
    //Float_t*  phoRandConeChIso = data.GetPtrFloat("phoRandConeChIso");
	...
	Phot_DY.SetPtEtaPhiE(PhoEt[acc_photon[g]], PhoEta[acc_photon[g]], PhoPhi[acc_photon[g]],PhoE[acc_photon[g]]);
	for (Int_t k=0; k<nMC; ++k){
	  if (fabs(mcPID[k]) == 22&&fabs(mcMomPID[k]) < 24){
	    Phot_MC.SetPtEtaPhiE(mcPt[k], mcEta[k], mcPhi[k], mcE[k]);
	    
	    if (fabs(Phot_DY.DeltaR(Phot_MC)) < 0.1) check_Zg =1;
	  }
	}
      }
    }
    if(check_Zg==1) DYTest++;
    ...
    */
	...
	if(phoInd >= 0){
      ...
      //if (fabs(phoSCEta[acc_photon[g]])>1.4442) continue;
      Float_t CorrEg = obj->GetCorrEtData((double)phoR9[acc_photon[g]], 2012, (double)PhoEt[acc_photon[g]], (double)phoSCEta[acc_photon[g]]);
      //if(fabs(PhoEt[acc_photon[g]]-CorrEg)>1) cout<<"b : "<<PhoEt[acc_photon[g]]<<"\t a : "<<CorrEg<<endl;
      if (CorrEg<15) continue;
      //if (Phot.Et()<15) continue;
      ...
      if (deltamin < 0.7) continue;
      //if (deltaR(mu1_wa.Eta(),mu1_wa.Phi(),Phot.Eta(),Phot.Phi()) < 0.7||deltaR(mu2_wa.Eta(),mu2_wa.Phi(),Phot.Eta(),Phot.Phi()) < 0.7) continue;
      //if (phoEleVeto[acc_photon[g]]!=0) continue;
      if (phohasPixelSeed[acc_photon[g]]!=0) continue;
      //if (Z.M()>120||Z.M()<60) continue;
      //if (Mmumu.M()+Z.M()>180) continue;
      if(count_pho==1){
	Phot_wa2.SetPtEtaPhiM(CorrEg, PhoEta[acc_photon[g]], PhoPhi[acc_photon[g]],0);
	SCEta_wa2 = phoSCEta[acc_photon[g]];
	phoSCRChIsoR2 = phoSCRChIso[acc_photon[g]];
	...
	count_pho++;
      ...
      if(count_pho==0){
	Phot_wa1.SetPtEtaPhiM(CorrEg, PhoEta[acc_photon[g]], PhoPhi[acc_photon[g]],0);
        SCEta_wa1 = phoSCEta[acc_photon[g]];
	phoSCRChIsoR1 = phoSCRChIso[acc_photon[g]];
    ...
    Photon1_Pt->Fill(Phot_wa1.Pt(),SF_ModAll*puwei);
    Photon2_Pt->Fill(Phot_wa2.Pt(),SF_ModAll*puwei);
    Zmass->Fill(Z.M(),SF_ModAll*puwei);
    Zgamma_mass->Fill(ZgammaL.M(),SF_ModAll*puwei);
    Zgammagamma_mass->Fill(ZgammagammaL.M(),SF_ModAll*puwei);
    two_mass->Fill(tpL.M(),SF_ModAll*puwei);
    Number_event->SetBinContent(1,Number_event->GetBinContent(1)+1);
    ...
    if (data.HasMC()) {
      for (Int_t k=0; k<nMC; ++k) {
	//if(mcPt[k]>15&&mcPID[k]==22&&mcParentage[k]!=2&&mcParentage[k]!=10) cout<<mcPID[k]<<"\t"<<mcMomPID[k]<<"\t"<<mcGMomPID[k]<<"\t"<<mcParentage[k]<<endl;
	if (fabs(mcPID[k]) == 22 && (mcParentage[k]==10||mcParentage[k]==18||mcParentage[k]==26)) {
	  Phot_MC.SetPtEtaPhiE(mcPt[k], mcEta[k], mcPhi[k], mcE[k]);
	...
	if (fabs(mcPID[k]) == 22 && mcParentage[k]==2){
	  Phot_MC.SetPtEtaPhiE(mcPt[k], mcEta[k], mcPhi[k], mcE[k]);
  ...
  Char_t fnameEt[11];
  sprintf(fnameEt,"EtDifCheck.txt");
  ofstream foutEt(fnameEt);
  cout<<pho1EtOvector.size()<<endl;
  for(Int_t k = 0;k < pho1EtOvector.size();k++){
    foutEt<<"Pho1 Orin : "<<pho1EtOvector[k]<<"\t Pho1 Aftr : "<<pho1EtAvector[k];
    foutEt<<"\t Pho2 Orin : "<<pho2EtOvector[k]<<"\t Pho2 Aftr : "<<pho2EtAvector[k]<<endl;
  }
  foutEt.close();

  //  cout<<"DYTest : "<<DYTest<<endl;
